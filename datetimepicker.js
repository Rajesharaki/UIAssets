        ...
        return this.each(function () {
            ...
            elem.on('click', function () {
                ...
                if (settings.showTime) {
					...
					var $ampm =  $fieldTime.find('#d-ampm');
                }
                ...
                function appendIncrement(typeDigits, increment) {
					if(typeDigits!=='ampm'){ 
							let $i = typeDigits == "hour" ? $hour : $minute;
                ...
                function createContent() {
                    ...
                    if (settings.showTime) {
                        ...
                        $line.text(lastSelected.format('YYYY'));
                        $a.append($line);
						$line = $('<div>');
                        $line.attr('id', 'time-line1');
                        $line.addClass('dtp_modal-time-line');
                        $line.text(lastSelected.format('ddd MMMM DD'));
                        $a.append($line);
						$line = $('<div>');
                        $line.attr('id', 'time-line2');
                        $line.addClass('dtp_modal-time-line');
                        $line.text(lastSelected.format('hh:MM A'));
                        $a.append($line);
                        ...
                        $el.append($but);
						var $but1 = $('<div>');
						$but1.addClass('dpt_modal-button');
                        $but1.text(settings.buttonTitle1);
                        $but1.bind('click', close);
                        $el.append($but1);                       
                        $c.append($el);
                    }
                    return $c;
                ...
                function updateDate() {
                    if (settings.showTime) {
                        $('#time-line1').text(lastSelected.format('ddd MMMM DD'));
                    }
                    updateMainElem();
